...
base::Value ViewCounterService::GetCurrentWallpaper() const {
  ...
  // In here, we should not return ntp si date if referral component is not yet
  // initialized. In that case, return base::Value().
  // We should give si wallpaper if we can confirm that this install is not
  // super-referral install.
