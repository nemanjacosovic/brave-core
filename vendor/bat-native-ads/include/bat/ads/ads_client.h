...
using OnIsParticipatingSiteForPublisherAdsCallback = std::function<void(
...
class ADS_EXPORT AdsClient {
 public:
  ...
  // |is_participating| should specify if the url supports publisher ads
  virtual void IsParticipatingSiteForPublisherAds(
      const std::string& url,
      const OnIsParticipatingSiteForPublisherAdsCallback callback) = 0;
